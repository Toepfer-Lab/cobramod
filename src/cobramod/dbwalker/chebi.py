...
def getChebiID(
    identifier: str, identifier_type: Literal["SMILES", "InChI", "InChIKey"]
) -> str:
    """
    ...
    logger.info(
        f"Requesting ChEBI ID for {identifier} of type {identifier_type}."
    )
    ...
    if response is None:
        logger.warning(
            f"There is no entry for {identifier} of type {identifier_type}."
        )
        return response
    ...
    if len(response) > 1:
        logger.warning(
            f"Multiple results found for {identifier} of type {identifier_type}. Returning first result."
        )
