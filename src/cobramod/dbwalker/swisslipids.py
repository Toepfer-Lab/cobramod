...
def get_swisslipids_id(identifier: str, identifier_type: str = "auto") -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"Requesting SwissLipids ID for {identifier_type}: {identifier}")
        
        response = requests.get(url, timeout=30)

        if response.status_code == 404 and response.json()["message"] == "Sorry, no result found":
            logger.error(f"No result available in database for: {identifier_type}={identifier}")
            return None
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            logger.info(f"Found {len(data)} results for {identifier_type}: {identifier}")
...
def get_swisslipids_data(swisslipids_id: str) -> GenerellIdentifiers:
    ...
    try:
        ...
        url = f"https://www.swisslipids.org/api/index.php/entity/{swisslipids_id}"
        
        logger.info(f"Requesting data from SwissLipids for ID: {swisslipids_id}")
        
        response = requests.get(url, timeout=30)

        if response.status_code == 500 and response.json()["message"] == "ERROR: entity is unknown":
