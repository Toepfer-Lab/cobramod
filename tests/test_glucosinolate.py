...
class GlucosinolateTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ignore_list = [
            ...
            "2_Oxo_carboxylates_c", "Oxidized_hemoproteins_c"]
        self.avoid_list = [
    ...
    def test_E_biomass(self):
        ...
        BIOMASS = 1
        with open(
                file=dir_test_case.joinpath("Bio_opt_rxn.txt"), mode="r") as f:
            for line in f:
                ...
                if float(line[1]) > 0 or line[0] == "MALONYL_ACP_p":
                    ...
                    test_biomass.add_metabolites({
                        test_model.metabolites.get_by_id(line[0]):
                        float(line[1])/BIOMASS
                    })
        ...
        self.assertAlmostEqual(
            test_fluxes.fluxes[
                "RIBULOSE_BISPHOSPHATE_CARBOXYLASE_RXN_p"], 16.981438106067703,
            delta=0.01)
        self.assertAlmostEqual(
            test_fluxes.fluxes["RXN_961_p"], 5.660479368689235,
            delta=0.01)
        self.assertAlmostEqual(
            test_fluxes.fluxes["H2O_tx"], 13.016328186042248,
            delta=0.01)
        self.assertAlmostEqual(
            test_fluxes.fluxes["O2_tx"], -20.73902048985689,
            delta=0.01)
        self.assertAlmostEqual(
            test_fluxes.fluxes["SO4_tx"], 0.058057461417529495,
            delta=0.01)
