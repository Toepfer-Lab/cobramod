...
class TestGetPubChem(TestCase):
    ...
    def test_getGenerellIdentifier(self):
        ...
        with patch.object(self.pubchem, "session", mock_session):
            ...
            self.assertEqual("InChI=1S/C2H6O/c1-2-3/h3H,2H2,1H3", gID_cached.inchi)
    ...
    def test_getDBIdentifierFromSmiles(self):
        ...
        with patch.object(self.pubchem, "session", mock_session):
            self.pubchem._cache.clear_cache()
            pubchemID = self.pubchem.getDBIdentifierFromSmiles("C1[C@H]([C@@H]([C@@H]([C@H](C(O1)OO)O)O)O)O")
    ...
    def test_getDBIdentifierFromInchi(self):
        ...
        with patch.object(self.pubchem, "session", mock_session):
            self.pubchem._cache.clear_cache()
            pubchemID = self.pubchem.getDBIdentifierFromInchi("InChI=1S/C2H6O/c1-2-3/h3H,2H2,1H3")
            self.assertEqual("702", pubchemID)
            mock_session.post.assert_called_once()
        ...
        with patch.object(self.pubchem, "session", mock_session):
            self.pubchem._cache.clear_cache()
            pubchemID = self.pubchem.getDBIdentifierFromInchi("InChI=1S/INVALID")
