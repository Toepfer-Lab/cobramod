...
class TestGetSwissLipidsId(TestCase):
    ...
    def test_get_id_from_inchikey_success(self):
        ...
        mock_response_data = [{"entity_id":"SLM:000000651","entity_name":"1-hexadecanoyl-2-(9Z-octadecenoyl)-sn-glycero-3-phosphocholine","external_id":"","entity_type":"metabolite","nb_exp_annot":0,"classification_level":"Isomeric subspecies","stats":{"entity_id":"SLM:000000651","nb_cheminfo":"0","nb_classification":"0","nb_metabolism":"52","nb_infered_metabolism":"0","nb_functions":"0","nb_go":"0","nb_interactions":"0","nb_locations":"0","nb_genetics":"0","nb_total":"52"}}]
        ...
        with patch('requests.get') as mock_get:
            ...
            result = get_swisslipids_id('WQZGKKKJIJFFOK-GASJEMHNSA-N', 'inchikey')
    ...
    def test_get_id_from_smiles_success(self):
        ...
        mock_response_data = [{"entity_id":"SLM:000597962","entity_name":"propyl propanoate","external_id":"","entity_type":"metabolite","nb_exp_annot":0,"classification_level":"not classified","stats":{"entity_id":"SLM:000597962","nb_cheminfo":"1","nb_classification":"1","nb_metabolism":"1","nb_infered_metabolism":"0","nb_functions":"0","nb_go":"0","nb_interactions":"0","nb_locations":"0","nb_genetics":"0","nb_total":"1"}}]
    ...
    def test_non_existing_id(self):
        ...
        with patch('requests.get') as mock_get:
            mock_response = Mock()
            mock_response.json.return_value = {"status":404,"message":"Sorry, no result found","title":"ERROR"}
            mock_response.status_code = 404
            mock_response.raise_for_status.return_value = 404
            mock_get.return_value = mock_response

            with self.assertLogs('cobramod.DBWalker.SwissLipids', level='ERROR') as cm:
                result = get_swisslipids_id('NONEXISTENT', 'SMILES')

                self.assertIsNone(result)
                self.assertEqual(cm.output, ["ERROR:cobramod.DBWalker.SwissLipids:No result available in database for: smiles=NONEXISTENT"])
    ...
    def test_live_api_integration(self):
            ...
            self.assertEqual(result.inchi, "1S/C6H12O6/c7-1-2-3(8)4(9)5(10)6(11)12-2/h2-11H,1H2/t2-,3-,4+,5-,6?/m1/s1")
            self.assertEqual(result.smiles, "OC[C@H]1OC(O)[C@H](O)[C@@H](O)[C@@H]1O")
...
class TestGetSwissLipidsData (TestCase):
    ...
    def test_get_data_success_all_identifiers(self):
        ...
        with patch('requests.get') as mock_get:
            ...
            self.assertIsInstance(result, GenerellIdentifiers)
            self.assertEqual("1S/C6H12O6/c7-1-2-3(8)4(9)5(10)6(11)12-2/h2-11H,1H2/t2-,3-,4+,5-,6?/m1/s1", result.inchi)
            self.assertEqual("WQZGKKKJIJFFOK-GASJEMHNSA-N", result.inchi_key)
            self.assertEqual("OC[C@H]1OC(O)[C@H](O)[C@@H](O)[C@@H]1O", result.smiles)
            mock_get.assert_called_once_with(
    ...
    def test_get_data_api_error(self):
        ...
        with patch('requests.get') as mock_get:
            ...
            with self.assertLogs('cobramod.DBWalker.SwissLipids', level='ERROR') as cm:
                ...
                self.assertEqual(cm.output, ["ERROR:cobramod.DBWalker.SwissLipids:Error fetching data from SwissLipids: API Error"])
