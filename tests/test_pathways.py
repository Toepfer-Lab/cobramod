...
from cobramod.creation import meta_string_to_model, add_meta_from_file,\
    _add_reaction_line_to_model, add_reaction
...
class ModulTesting(unittest.TestCase):
    ...
    def test__fix_meta_for_boundaries(self):
        ...
        add_reaction(
            model=test_model, identifier="OXALODECARB-RXN",
            directory=dir_data, database="META",
            compartment="c", replacement_dict={})
        self.assertRaises(
    ...
    def test__verify_side_sinks_for_rxn(self):
        ...
        test_model = cb.Model(0)
        add_reaction(
            model=test_model, identifier="OXALODECARB-RXN", directory=dir_data,
            database="META", compartment="c", replacement_dict={})
    ...
    def test_verify_sinks_for_rxn(self):
        ...
        add_reaction(
            model=test_model, identifier="OXALODECARB-RXN",
            directory=dir_data, database="META", compartment="c",
            replacement_dict={})
    ...
    def test__test_rxn_for_solution(self):
        ...
        test_model = cb.Model(0)
        add_reaction(
            model=test_model, identifier="RXN-2206", directory=dir_data,
            database="META", compartment="c", replacement_dict={})
        ...
        test_model = cb.Model(0)
        add_reaction(
            model=test_model, identifier="1.8.4.9-RXN", directory=dir_data,
            database="META", compartment="c", replacement_dict={})
    ...
    def test__add_sequence(self):
        ...
        meta_string_to_model(
            line="WATER, c", model=test_model, directory=dir_data,
            database="META")
        meta_string_to_model(
            line="OXYGEN-MOLECULE, c", model=test_model, directory=dir_data,
            database="META")
        ...
        _add_reaction_line_to_model(
            ...
            model=test_model, directory=dir_data,
            database="META")
        test_model.objective = "Dummy_c"
        test_list = list(pt._create_reactions(
        ...
        test_reactions = list(
            pt._create_reactions(
                sequence=test_list[1], directory=dir_data,
        ...
        test_reactions = list(
            pt._create_reactions(
                sequence=test_list[2], directory=dir_data,
                compartment="c",
                database="META"))
        pt._add_sequence(
            model=test_model, sequence=test_reactions,
    ...
    def test__add_graph_to_model(self):
        ...
        pt.add_graph_to_model(
            model=test_model, graph="PWY-1187", compartment="c",
            directory=dir_data, database="META",
            ignore_list=new_sink_list,
        )
        self.assertIn(
        ...
        pt.add_graph_to_model(
            model=test_model, graph="AMMOXID-PWY", compartment="c",
            directory=dir_data, database="META",
            ignore_list=new_sink_list,
        )
        self.assertGreater(abs(test_model.slim_optimize()), 0)
        ...
        test_model.objective = "Dummy_c"
        self.assertGreater(
            a=test_model.slim_optimize(),
            b=0)
        test_sequence = set([
            "RXN-2206", "RXN-11414", "RXN-11422", "RXN-11430"])
        pt.add_graph_to_model(
            model=test_model, compartment="c", graph=test_sequence,
            ignore_list=new_sink_list, database="META", directory=dir_data)
        ...
        self.assertIn(
            ...
            container=[reaction.id for reaction in test_model.reactions])
        self.assertGreater(
            a=test_model.optimize().fluxes["RXN_11430_c"],
            b=0)
